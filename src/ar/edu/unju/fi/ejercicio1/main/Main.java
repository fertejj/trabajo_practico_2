...
import ar.edu.unju.fi.ejercicio1.model.Producto.FabricadoEn;
...
public class Main {
	...
	public static void crearProducto(Scanner scanner, ArrayList<Producto> productos) {
		...
		try {
			...
			FabricadoEn eleccionOrigen = menuOrigen(scanner);
	...
	public static FabricadoEn menuOrigen(Scanner scanner) {
		FabricadoEn[] origenes = FabricadoEn.values();
		FabricadoEn eleccion = null;
		int opcion;
		Boolean verif = false;
		while(verif == false) {
			...
			try {
				...
				if (verif == true) {
					for (FabricadoEn origen : origenes) {
	...
	public static void Modificar(Scanner scanner, Producto producto, int eleccion) {
		switch(eleccion) {
		    ...
		    case 3:
		    	FabricadoEn eleccionOrigen = menuOrigen(scanner);
		    	producto.setFabricadoEn(eleccionOrigen);
		    	break;
